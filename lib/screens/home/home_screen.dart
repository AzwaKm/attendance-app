...
class _HomeScreenState extends State<HomeScreen> {
  ...
  // mendengarkan segala hal yang terjadi di home screen
  void _listenToTodayRecord() {
    final user = _authServices.currentUser;
    if (user != null) {
      _firestoreServices.getTodayRecordStream(user.uid).listen((record) {
  ...
  Future<void> _checkIn({String? photoPath}) async {
    ...
    try {
      String? photoKey;
      if (photoPath != null) {
        photoKey = await _storageServices.uploadAttendancePhoto(photoPath, 'Check In');
      }
      ...
      await _firestoreServices.AttendanceRecords(record);
  ...
  Future<void> _checkOut({String? photoPath}) async {
    ...
    try {
      ...
      if (photoPath != null) {
        ...
        await _firestoreServices.updateAttendanceRecord(updateRecord);
